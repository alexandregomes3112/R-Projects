#*************************************************************************
#
#             Microsoft Power BI Para Data Science, v 2.0
#
...
setwd("C:/Users/BIJ/Dropbox/EPSON/RH/Power BI/DSA/Cap15")
getwd()

# Definição do Problema
# Leia o manual em pdf no Capítulo 15 do curso com a definição do problema
...
# Veja % entre as classes
prop.table(table(dados_treino$Inadimplente))

# Número de registros no dataset de treinamento
dim(dados_treino)

# Comparamos as % entre as classes de treinamento e dados originais
...
# Tudo o que nao esta no dataset de treinamento esta no dataset de teste. Observe o dinal - (menos)
...
# Aplicando o SMOTE - SMOTE: Systetic Minority Over-Sampling Technique
# https://arxiv.org/pdf/1106.1813.pdf
table(dados_treino$Inadimplente)
prop.table(table(dados_treino$Inadimplente))
set.seed(9560)
dados_treino_bal <- SMOTE(Inadimplente ~ ., data = dados_treino)
table(dados_treino_bal$Indadimplente)
...
# Criando o rank de variÃ¡veis baseado na importÃ¢ncia
rankImportance <- varImportance %>% 
  mutate(Rank = paste0('#', dense_rank(desc(Importance))))

# Usando ggplot2 para visualizar a importÃ¢ncia relativa das variÃ¡veis
...
# Calculando Precision, Recall e F1-Score, métricas de avaliaçãoo do modelo preditivo
y <- dados_teste$Inadimplente
